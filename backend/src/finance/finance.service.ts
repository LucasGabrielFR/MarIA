...
export class FinanceService {
  ...
  async getFinanceSummary(startDate?: string, endDate?: string) {
    ...
    if (!startDate && !endDate) {
      ...
      if (error) {
        this.logger.error(
          `Erro ao buscar sumário financeiro: ${error.message}`,
        );
        return {
          total_revenue: 0,
          total_cost: 0,
    ...
    let queryLogs = client.from('usage_logs').select('cost');
    ...
    if (subRes.error) {
      this.logger.error(
        `Erro ao agregar faturamento no período: ${subRes.error.message}`,
      );
    }
    if (logsRes.error) {
      this.logger.error(
        `Erro ao agregar custos de IA no período: ${logsRes.error.message}`,
      );
    }

    const total_revenue = (subRes.data || []).reduce(
      (acc, sub) => acc + Number(sub.amount),
      0,
    );
    const total_cost = (logsRes.data || []).reduce(
      (acc, log) => acc + Number(log.cost),
      0,
    );
  ...
  private async validateSuperAdmin(adminEmail: string) {
    if (!adminEmail) {
      throw new ForbiddenException(
        'Acesso negado: Administrador não identificado.',
      );
    }
    ...
    if (error || !admin || admin.role !== 'superadmin') {
      ...
      throw new ForbiddenException(
        'Apenas superadministradores podem realizar esta ação.',
      );
    }
  }
  ...
  async cancelSubscription(subscriptionId: string, adminEmail: string) {
    ...
    const { data: sub, error: subError } = await this.supabaseService
      .getClient()
      .from('subscriptions')
    ...
    if (subError) {
      this.logger.error(
        `Erro ao cancelar assinatura ${subscriptionId}: ${subError.message}`,
      );
      throw subError;
    }
    ...
    if (sub && sub.user_id) {
      ...
      if (userError) {
        this.logger.error(
          `Erro ao revogar assinatura do usuário ${sub.user_id}: ${userError.message}`,
        );
        throw userError;
      }
    }
  ...
  async deleteSubscription(subscriptionId: string, adminEmail: string) {
    ...
    const { data: sub, error: fetchError } = await this.supabaseService
      .getClient()
      .from('subscriptions')
      .select('user_id')
      .eq('id', subscriptionId)
    ...
    if (fetchError) {
      this.logger.error(
        `Erro ao buscar assinatura ${subscriptionId} para deleção: ${fetchError.message}`,
      );
      throw fetchError;
    }
    ...
    if (deleteError) {
      this.logger.error(
        `Erro ao deletar assinatura ${subscriptionId}: ${deleteError.message}`,
      );
      throw deleteError;
    }
    ...
    if (sub && sub.user_id) {
      ...
      if (userError) {
        this.logger.error(
          `Erro ao revogar assinatura do usuário ${sub.user_id} após deleção: ${userError.message}`,
        );
        throw userError;
      }
    }
  ...
  async updateSubscriptionInAsaas(
    ...
    if (fetchError || !sub) {
      this.logger.error(
        `Erro ao buscar assinatura ${subscriptionId}: ${fetchError?.message}`,
      );
    ...
    if (!user || !user.asaas_subscription_id) {
      throw new Error(
        'Usuário associado não possui assinatura no Asaas ativa.',
      );
    }
    ...
    if (updateError) {
      this.logger.error(
        `Erro ao atualizar assinatura no banco: ${updateError.message}`,
      );
      throw updateError;
    }
